...
const MuiEndButtonIcon = (props) => {
  const {color,endIcon,variant,children,isDisable,className,handleClick,} = props;

  return (
    <Stack spacing={2} direction="row">
      <Button color={color} endIcon={endIcon} variant={variant} disabled={isDisable} onClick={handleClick} className={className}>
        {children}
      </Button>
    </Stack>
  );
